...
if __name__ == "__main__":
    ...
    core = ScraperCore()
    id_url_map = core.charger_liens_avec_id()
    id_list = core.charger_liste_ids()
    ...
    def execute_actions():
        ...
        selected_base = base_dir_var.get() or core.base_dir
        core.prepare_results_dir(selected_base, result_name)

        actions = []
        if var_variantes.get():
            actions.append(lambda: core.scrap_produits_par_ids(id_url_map, ids))
        if var_concurrents.get():
            actions.append(lambda: core.scrap_fiches_concurrents(id_url_map, ids))
        if var_json.get():
            actions.append(core.export_fiches_concurrents_json)
        ...
        def run_all():
            ...
            summary = (
                ...
                f"📁 Données dans: {core.results_dir}"
            )
            print(summary)
            try:
    ...
    def open_results_folder():
        import subprocess
        import platform
        default_path = os.path.join(base_dir_var.get() or core.base_dir, "fiche concurrents")
        path = core.results_dir if core.results_dir else default_path
    ...
    base_dir_var = tk.StringVar(value=core.base_dir)
    ...
    fiches_dir_var = tk.StringVar(value=os.path.join(core.base_dir, "fiche concurrents"))
